...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if ($action === 'create') {
        ...
        elseif (!$content) { $error = 'Content is required.'; }
            ...
            execute("INSERT INTO notices (title, content, image_url, target_pages, is_active, starts_at, ends_at, created_by) VALUES (?, ?, ?, ?, ?, ?, ?, ?)",
    ...
    elseif ($action === 'delete') {
        execute("DELETE FROM notices WHERE id=?", [$_POST['id']]);
        $success = 'Notice deleted.';
    }
    elseif ($action === 'toggle') {
        execute("UPDATE notices SET is_active=NOT is_active WHERE id=?", [$_POST['id']]);
...
<head>
    ...
    <title>Notice Management — <?= SITE_NAME ?></title>
    <link rel="stylesheet" href="<?= $__asset('/assets/css/fonts.css') ?>">
    <link rel="stylesheet" href="<?= $__asset('/assets/theme.css') ?>">
    <link rel="stylesheet" href="<?= $__asset('/assets/css/admin.css') ?>">
    <link rel="stylesheet" href="<?= $__asset('/assets/css/admin-forms.css') ?>">
...
<body class="bg-base-200 min-h-screen">
    <?php include __DIR__ . '/../includes/admin-layout-start.php'; ?>
    ...
    <div class="max-w-5xl mx-auto">
        ...
        <!-- Notices List -->
        <div class="space-y-4">
            <?php while ($n = $notices->fetch(PDO::FETCH_ASSOC)): ?>
            <div class="card bg-base-100 shadow-sm <?= !$n['is_active'] ? 'opacity-60' : '' ?>">
                <div class="card-body p-5">
                    <div class="flex gap-4">
                        <?php if ($n['image_url']): ?>
                        <div class="notice-preview w-32 flex-shrink-0">
                            <img src="<?= e($n['image_url']) ?>" alt="" onerror="this.parentElement.innerHTML='<div class=\'h-full flex items-center justify-center bg-muted text-muted-foreground text-xs\'>No image</div>'">
                        </div>
                        <?php endif; ?>
                            ...
                            <div class="flex items-start justify-between gap-4">
                                <div>
                                    ...
                                    <div class="target-tags mt-1">
                                        <?php 
                                        $pages = explode(',', $n['target_pages']);
                                        foreach ($pages as $p): $p = trim($p);
                                            $cls = $p === 'all' ? 'target-all' : ($p === 'public' ? 'target-public' : ($p === 'admin' ? 'target-admin' : ($p === 'client' ? 'target-client' : 'bg-muted text-muted-foreground')));
                                            echo "<span class='target-tag $cls'>" . e(strtoupper($p)) . "</span>";
                                        endforeach; 
                                        ?>
                                    </div>
                                </div>
            ...
            <form method="POST">
                ...
                <div class="form-group mb-4">
                    ...
                    <select name="target_pages" class="form-select">
                        ...
                        <option value="portal">Client Portal Pages</option>
                    </select>
                </div>
